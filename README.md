...
6.反思与改进
-----
由于初学，本代码仅仅实现基本功能，对于进阶功能还没有实现<dr>
* 识别装甲板中心数字，实现机器人 ID 分类（1、2、3、4、Sentry）<dr>
* 估算装甲板的三维空间姿态（旋转角度、俯仰角等）<dr>
* 能够预测装甲板的运动(引入卡尔曼滤波器进行观测)<dr>
并且用了传统opencv识别的方案，识别效率不高，且容易受光线等因素影响，当运动过快时，也会出现错误识别，错误配对，框选的区域并不是装甲板等漏洞，还需改进。
